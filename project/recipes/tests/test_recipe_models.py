...
class RecipeModelTest(RecipeTestBase):
    ...
    def make_recipe_no_defaults(self):
        recipe = Recipe(
            ...
            author = self.make_author(username="User test"),
            title = "Recipe title",
    ...
    def test_recipe_fields_max_length(self, field, max_length):
        setattr(self.recipe, field, "R" * (max_length + 1))
        with self.assertRaises(ValidationError):
            self.recipe.full_clean() 
        
    def test_recipe_preparation_steps_is_html_is_false_by_dafault(self):
        recipe = self.make_recipe_no_defaults()
        self.assertFalse(recipe.preparation_steps_is_html, msg="Recipe preparation_steps_is_html is not False")

    def test_recipe_is_published_is_false_by_dafault(self):
        recipe = self.make_recipe_no_defaults()
